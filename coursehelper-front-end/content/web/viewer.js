...
var DocumentOutlineView = function documentOutlineView(outline) {
  ...
  (function rewriteUrlClosure() {
    ...
    //改这里了,如果有index参数，做处理
    if(DEFAULT_URL.indexOf("&index=")>0){
      var index = DEFAULT_URL.substring(DEFAULT_URL.indexOf("&index="));
      noteIndex = index.substring(index.indexOf("=")+1);
      DEFAULT_URL = DEFAULT_URL.substring(0,DEFAULT_URL.indexOf("&index="));
    }
...
function webViewerLoad(evt) {
  ...
  if('page' in hashParams){
    PDFView.initialBookmark = "page="+hashParams['page'];
  }
...
//改这里了
  if(canSend){
    if(FIRSTUPDATE){
      ...
      if(noteIndex){
        var iframedata = new Object();
        iframedata.key="noteIndex" ;
        iframedata.value=noteIndex ;
        window.frames[0].postMessage(iframedata,ToServer);
      }
...
window.addEventListener('message',function(e) {
    var msg = e.data ;
    if(msg.key == "closeRecord"){
        $.modal.close();
        //发表成功，通知本页刷新
        window.frames[0].postMessage({key:"update"},ToServer);
    }
    else if(msg.key == "selection"){
        rangy.deserializeSelection(msg.value);
    }
    else if(msg.key == "clearSelection"){
...
window.onload = function(){
  ...
  if(('logged' in localStorage) && localStorage.logged == "true"){
    ...
    iframe.src = ToServer+"/NoteDisplay" ;
    iframe.id = "helper";
